...
| Preparação/instalação/trabalhos culturais | parcial | várias ações, custom local reutilizável, densidade e Retancha cumprem; a base ainda não tem uma entidade administrável para desativar itens custom nem associação de análise de solo à preparação |
...
| Recursos/custos/caderno | parcial | custos por área/override, snapshots e reversão cumprem; recursos comuns ficam ligados uma vez à operação, mas horas/alocação explícita por destino ainda não são persistidas |
...
Os dois estados `parcial` remanescentes representam capacidades de produto ainda sem modelo persistente na base auditada; não foram simuladas com JSON livre nem com integrações inventadas. Todos os desvios que afetavam invariantes já representados e fluxos executáveis foram corrigidos.
