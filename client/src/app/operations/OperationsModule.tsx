import{useCallback,useEffect,useMemo,useState,type FormEvent}from"react";import type{FieldDto}from"@shared/fields";import type{PlantationDto}from"@shared/crop-lifecycle";import type{ContractorDto,EquipmentDto,WorkerDto}from"@shared/resources";import{culturalWorkActionIds,soilPreparationActionIds,type OperationDto}from"@shared/operations";import type{CultureCatalogEntry,VarietyDto}from"@shared/crops";import{useI18n}from"../../i18n";import{operationCopies,type OperationCopy}from"./operation-locales.generated";import"./operations.css";import"./cultural-work.css";import"./fertilization.css";type Data={operations:OperationDto[];fields:FieldDto[];plantations:PlantationDto[];workers:WorkerDto[];equipment:EquipmentDto[];contractors:ContractorDto[];cultures:readonly CultureCatalogEntry[];varieties:VarietyDto[];};type DestinationForm={fieldId:string;plantationId:string;areaHa:string;percentage:string;};type MaterialLotForm={varietyId:string;lotNumber:string;quantity:string;unit:string;origin:string;supplier:string;};type InstallationForm={plantationName:string;cultureId:string;varietyIds:string[];varietyDensities:Record<string,string>;kind:"permanent"|"temporary";endedOn:string;method:"sowing"|"transplanting"|"planting"|"other";customMethod:string;densityPlantsHa:string;rowSpacingCm:string;plantSpacingCm:string;materialLots:MaterialLotForm[];predecessor:string;preparatoryOperationIds:string[];};type CulturalMaterialForm={name:string;quantity:string;unit:string;lotNumber:string;};type CulturalForm={actions:string[];customAction:string;method:"manual"|"mechanical"|"thermal"|"other";customMethod:string;intensity:""|"light"|"medium"|"severe";intensityPercentage:string;biomassDestination:string;plantPercentage:string;plantCount:string;materials:CulturalMaterialForm[];originalDensityPlantsHa:string;plantsReplaced:string;plantsPlaced:string;estimatedCurrentDensityPlantsHa:string;};type FertilizerProductForm={name:string;category:"fertilizer"|"amendment"|"organic";quantitySource:"dose_per_ha"|"total";dosePerHa:string;totalQuantity:string;unit:"kg"|"l"|"t";densityKgL:string;lotNumber:string;compositionKnown:boolean;dryMatterPercent:string;nTotal:string;nNitrate:string;nAmmonium:string;nUreic:string;nOrganic:string;p2o5:string;k2o:string;cao:string;mgo:string;so3:string;organicMatter:string;carbon:string;micronutrients:string;destinationDoses:Record<string,string>;};type FertilizationForm={mode:"base"|"top_dressing"|"foliar"|"amendment"|"organic_matter"|"cover_crop_incorporation"|"other";customMode:string;products:FertilizerProductForm[];};const empty:Data={operations:[],fields:[],plantations:[],workers:[],equipment:[],contractors:[],cultures:[],varieties:[]};export default function OperationsModule(){const{locale}=useI18n();const t=operationCopies[locale];const[data,setData]=useState(empty);const[adding,setAdding]=useState(false);const[loading,setLoading]=useState(true);const[failed,setFailed]=useState(false);const load=useCallback(async()=>{setLoading(true);setFailed(false);try{const responses=await Promise.all(["/api/farm/operations","/api/farm/fields","/api/farm/crop-lifecycle","/api/farm/resources","/api/farm/crop-catalog","/api/farm/varieties"].map(url=>fetch(url,{credentials:"include"})));if(responses.some(response=>!response.ok))throw new Error();const[operations,fields,lifecycle,resources,cultures,varieties]=await Promise.all(responses.map(response=>response.json()));setData({operations:operations.data,fields:fields.data,plantations:lifecycle.data.plantations,workers:resources.data.workers,equipment:resources.data.equipment,contractors:resources.data.contractors,cultures:cultures.data,varieties:varieties.data});}catch{setFailed(true);}finally{setLoading(false);}},[]);useEffect(()=>{void load();},[load]);const fields=useMemo(()=>new Map(data.fields.map(item=>[item.id,item.name])),[data.fields]);const plantations=useMemo(()=>new Map(data.plantations.map(item=>[item.id,item.name])),[data.plantations]);return<>
      ...
      <section className="panel operation-panel">
        {loading?<div className="module-state">
            ...
            {data.operations.map(operation=><article key={operation.id}>
                ...
                <div>
                  <b>{typeLabel(t,operation.type)}</b>
                  <span>
      ...
      <section className="holding-dialog field-dialog">
        ...
        <form className="holding-form operation-form"onSubmit={event=>void submit(event)}>
          <div className="operation-destinations">
            {values.destinations.map((destination,index)=>{const compatible=data.plantations.filter(item=>item.fieldId===destination.fieldId&&item.status==="active");return<fieldset key={index}>
                  <label>
                    <span>{t.field}</span>
          ...
          <label>
            <span>{t.type}</span>
            <select value={values.type}onChange={event=>{const type=event.target.value;setValues(current=>({...current,type,...(type==="crop_installation"?{destinations:[{...current.destinations[0],plantationId:"",percentage:"100"}]}:{})}));}}>
              {(["soil_preparation","crop_installation","cultural_work","fertilization","spraying","irrigation","fertigation","monitoring","harvest","other"]as OperationDto["type"][]).map(type=><option key={type}value={type}>
                  {typeLabel(t,type)}
                </option>)}
            </select>
          </label>
    ...
    </div>;}function FertilizationFields({data,t,value,set,destinations,totalArea}:{data:Data;t:OperationCopy;value:FertilizationForm;set:<K extends keyof FertilizationForm>(key:K,value:FertilizationForm[K])=>void;destinations:DestinationForm[];totalArea:number;}){const update=(index:number,key:keyof FertilizerProductForm,next:string|boolean)=>set("products",value.products.map((product,itemIndex)=>{if(itemIndex!==index)return product;const changed={...product,[key]:next};if(key==="dosePerHa"&&typeof next==="string"){changed.quantitySource="dose_per_ha";changed.totalQuantity=totalArea&&next?String(Number(next)*totalArea):"";}if(key==="totalQuantity"&&typeof next==="string"){changed.quantitySource="total";changed.dosePerHa=totalArea&&next?String(Number(next)/totalArea):"";}return changed;}));const updateDestinationDose=(index:number,key:string,dose:string)=>set("products",value.products.map((product,itemIndex)=>itemIndex===index?{...product,destinationDoses:{...product.destinationDoses,[key]:dose}}:product));return<fieldset className="fertilization-fields">
      <legend>{t.fertilizationDetails}</legend>
      <div className="fertilization-grid">
        <label>
          <span>{t.applicationMode}</span>
          <select value={value.mode}onChange={event=>{const mode=event.target.value as FertilizationForm["mode"];set("mode",mode);if(mode!=="other")set("customMode","");}}>
    ...
    </fieldset>;}function CulturalWorkFields({data,t,value,set}:{data:Data;t:OperationCopy;value:CulturalForm;set:<K extends keyof CulturalForm>(key:K,value:CulturalForm[K])=>void;}){const customActions=[...new Set(data.operations.flatMap(item=>item.culturalWork?.actions||[]).filter(action=>!culturalWorkActionIds.includes(action as(typeof culturalWorkActionIds)[number])))].sort((a,b)=>a.localeCompare(b));const customMethods=[...new Set(data.operations.map(item=>item.culturalWork?.customMethod).filter((item):item is string=>Boolean(item)))];const toggleAction=(id:string)=>set("actions",value.actions.includes(id)?value.actions.filter(item=>item!==id):[...value.actions,id]);const updateMaterial=(index:number,key:keyof CulturalMaterialForm,next:string)=>set("materials",value.materials.map((item,itemIndex)=>itemIndex===index?{...item,[key]:next}:item));const replanting=value.actions.includes("replanting");return<fieldset className="cultural-work-fields">
      ...
      <div className="cultural-detail-grid">
        <label>
          ...
          <select value={value.method}onChange={event=>{const method=event.target.value as CulturalForm["method"];set("method",method);if(method!=="other")set("customMethod","");}}>
    ...
    </fieldset>;}function CropInstallationFields({data,t,value,set,toggleAction}:{data:Data;t:OperationCopy;value:InstallationForm;set:<K extends keyof InstallationForm>(key:K,value:InstallationForm[K])=>void;toggleAction:(id:string)=>void;}){const compatibleVarieties=data.varieties.filter(item=>item.cultureId===value.cultureId);const preparationOperations=data.operations.filter(item=>item.type==="soil_preparation"&&item.status==="performed");const customMethods=[...new Set(data.operations.map(item=>item.cropInstallation?.customMethod).filter((item):item is string=>Boolean(item)))];const toggleVariety=(id:string)=>{if(value.varietyIds.includes(id)){set("varietyIds",value.varietyIds.filter(item=>item!==id));const densities={...value.varietyDensities};delete densities[id];set("varietyDensities",densities);}else set("varietyIds",[...value.varietyIds,id]);};const updateLot=(index:number,key:keyof MaterialLotForm,next:string)=>set("materialLots",value.materialLots.map((lot,lotIndex)=>lotIndex===index?{...lot,[key]:next}:lot));const addLot=()=>set("materialLots",[...value.materialLots,{varietyId:"",lotNumber:"",quantity:"",unit:"kg",origin:"",supplier:""}]);return<fieldset className="crop-installation-fields">
      ...
      <div className="installation-grid">
        ...
        <label>
          ...
          <select value={value.method}onChange={event=>{const method=event.target.value as InstallationForm["method"];set("method",method);if(method!=="other")set("customMethod","");}}>
...
function FertilizationSummary({data,locale,t}:{data:Data;locale:string;t:OperationCopy;}){
  ...
  const totals=useMemo(()=>{
    const products=new Map<string,{name:string;unit:string;quantity:number}>();
    const nutrients=new Map<string,number>();
    for(const operation of fertilizations)for(const product of operation.fertilization!.products){
      ...
      for(const[key,value]of Object.entries(product.nutrientTotalsKg))nutrients.set(key,(nutrients.get(key)||0)+value*share);
    }
    return{products:[...products.values()],nutrients:[...nutrients.entries()]};
  },[fertilizations,fieldId,plantationId]);
  if(!data.operations.some(operation=>operation.fertilization))return null;
