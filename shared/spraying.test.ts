...
const applicatorId = "00000000-0000-4000-8000-000000000002";
...
test("a mixed spray tank remains one operation with several independently authorized products", () => {
  const value = sprayingSchema.parse({ method: "spray", sprayVolumeLHa: 500, legalApplicatorWorkerId: applicatorId, products: [
...
test("legal applicator is classification-driven and warnings never block persistence", () => {
  ...
  assert.throws(() => sprayingSchema.parse({ method: "spray", sprayVolumeLHa: 400, products: [{ name: "Fungicide", category: "phytopharmaceutical", registrationNumber: "AV-1", unit: "l", quantitySource: "total", totalQuantity: 1, authorizations: [authorization] }], weather: { source: "unavailable" } }));
  assert.throws(() => sprayingSchema.parse({ method: "spray", sprayVolumeLHa: 400, legalApplicatorWorkerId: applicatorId, auxiliaryWorkerIds: [applicatorId], products: [{ name: "Fungicide", category: "phytopharmaceutical", registrationNumber: "AV-1", unit: "l", quantitySource: "total", totalQuantity: 1, authorizations: [authorization] }], weather: { source: "unavailable" } }));
});

test("the specialist record is attached to exactly one physical operation",()=>{const spraying={method:"spray",sprayVolumeLHa:300,legalApplicatorWorkerId:applicatorId,products:[{name:"Sulfur",category:"phytopharmaceutical",unit:"kg",quantitySource:"dose_per_ha",dosePerHa:2,registrationNumber:"AV-2",authorizations:[authorization]}],weather:{source:"unavailable"}};const operation=createOperationSchema.parse({destinations:[{fieldId:id,areaHa:1,percentage:100}],type:"spraying",performedAt:"2026-08-24T10:00:00+01:00",spraying});assert.equal(operation.type,"spraying");assert.equal(operation.spraying?.products.length,1);assert.throws(()=>createOperationSchema.parse({...operation,type:"fertilization"}));});
