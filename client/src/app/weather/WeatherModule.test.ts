...
test("weather UI exposes local audit state and uses explicit POST synchronization actions",async()=>{const source=await readFile(new URL("./WeatherModule.tsx",import.meta.url),"utf8");assert.match(source,/agronomic-series/u);assert.match(source,/plantationId\}\/conditions/u);assert.match(source,/method: "POST", body: JSON\.stringify/u);assert.match(source,/item\.inputHash/u);assert.match(source,/item\.state !== "available"/u);assert.doesNotMatch(source,/conditions\?from|agronomic-series\?from|agronomic-accumulation|pending-core-weather|\/report/u)});
