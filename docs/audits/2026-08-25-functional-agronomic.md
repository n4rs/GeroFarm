...
| Preparação/instalação/trabalhos culturais | cumpre no domínio/API | catálogo organizacional persistente, reutilizável, desativável e sem delete; preparação seleciona a análise válida por destino/data, congela o boletim completo e avisa sem bloquear quando ausente. Gestão visual do catálogo e seletor explícito pertencem à Tarefa 3 |
...
Não permanecem requisitos funcionais locais classificados como `parcial`. A Tarefa 3 pode consumir `GET/POST/PATCH /api/farm/operation-catalog`, `soilAnalysisResultId`, os snapshots/warnings devolvidos e os contratos `workerAssignments`, `equipmentAssignments` e `contractorAssignments` para completar a experiência visual sem alterar estes invariantes.
