...
import { economicCopies } from "../economics/economic-locales";
...
export default function SprayingFields({ t, value, set, workers, validApplicatorIds, destinations, isSpraying }: Props) {
    const { locale } = useI18n(), operation = operationCopies[locale], economics = economicCopies[locale];
  ...
  <div className="fertilizer-products"><header><b>{t.applicationDetails}</b><button type="button" className="subtle-button" onClick={() => set("products", [...value.products, blankSprayProduct()])}>＋ {t.addProduct}</button></header>{value.products.map((product, index) => <fieldset key={index}>
   ...
   <label><span>{t.quantitySource}</span><select value={product.quantitySource} onChange={event => update(index, "quantitySource", event.target.value)}><option value="dose_per_ha">{operation.dosePerHa}</option><option value="dose_per_hl">{t.dosePerHl}</option><option value="total">{operation.totalQuantity}</option></select></label>{product.quantitySource === "dose_per_ha" && <Input label={operation.dosePerHa} type="number" value={product.dosePerHa} onChange={next => update(index, "dosePerHa", next)}/>} {product.quantitySource === "dose_per_hl" && <Input label={t.dosePerHl} type="number" value={product.dosePerHl} onChange={next => update(index, "dosePerHl", next)}/>} {product.quantitySource === "total" && <Input label={operation.totalQuantity} type="number" value={product.totalQuantity} onChange={next => update(index, "totalQuantity", next)}/>}<label><span>{operation.unit}</span><select value={product.unit} onChange={event => update(index, "unit", event.target.value)}>{["kg", "g", "l", "ml"].map(unit => <option key={unit} value={unit}>{unit === "l" ? "L" : unit}</option>)}</select></label>
   <Input label={operation.lotNumber} required={false} value={product.lotNumber} onChange={next => update(index, "lotNumber", next)}/><Input label={economics.unitCost} type="number" required={false} value={product.unitCost||""} onChange={next=>update(index,"unitCost",next)}/><Input label={t.activeSubstances} required={false} value={product.activeSubstances} onChange={next => update(index, "activeSubstances", next)}/>{product.category === "phytopharmaceutical" && <><Input label={t.registrationNumber} value={product.registrationNumber} onChange={next => update(index, "registrationNumber", next)}/><Input label={t.fracGroup} required={false} value={product.fracGroup} onChange={next => update(index, "fracGroup", next)}/></>}<Input label={t.targets} required={false} value={product.targets} onChange={next => update(index, "targets", next)}/>
   ...
   {(product.category === "foliar_fertilizer" || product.category === "corrective") && <><label><input type="checkbox" checked={product.compositionKnown} onChange={event => update(index, "compositionKnown", event.target.checked)}/><span>{operation.knownComposition}</span></label>{product.compositionKnown && <><Input label={operation.knownComposition} value={product.composition} onChange={next => update(index, "composition", next)}/>{product.unit === "l" && <Input label={operation.densityKgL} type="number" value={product.densityKgL} onChange={next => update(index, "densityKgL", next)}/>}</>}</>}
