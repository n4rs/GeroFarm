...
export const farmOperations=farmSchema.table("operations",{id:uuid("id").primaryKey(),organizationId:uuid("organization_id").notNull().references(()=>farmOrganizations.organizationId),code:varchar("code",{length:24}).notNull(),fieldId:uuid("field_id").notNull().references(()=>farmFields.id),plantationId:uuid("plantation_id").references(()=>plantations.id),type:varchar("type",{length:32}).notNull(),performedAt:timestamp("performed_at",{withTimezone:true}).notNull(),areaHa:numeric("area_ha",{precision:12,scale:4}),durationMinutes:numeric("duration_minutes",{precision:8,scale:0}),notes:varchar("notes",{length:2000}),status:varchar("status",{length:16}).notNull().default("performed"),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow()},(table)=>[uniqueIndex("operations_organization_code_unique").on(table.organizationId,table.code),index("operations_field_date_idx").on(table.organizationId,table.fieldId,table.performedAt),check("operations_status_valid",sql`${table.status} in ('performed','voided')`),pgPolicy("operations_tenant_isolation",{as:"restrictive",for:"all",to:"public",using:sql`${table.organizationId}=nullif(current_setting('app.organization_id',true),'')::uuid`,withCheck:sql`${table.organizationId}=nullif(current_setting('app.organization_id',true),'')::uuid`})]).enableRLS();export type FarmOperation=typeof farmOperations.$inferSelect;
