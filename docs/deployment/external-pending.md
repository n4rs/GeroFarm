...
- A meteorologia automática da monitorização reutiliza server-side a série base Weather v2 persistida da `plantation`, validada contra os talhões e o período cultural local. O cliente não pode enviar `automaticWeather`; o snapshot imutável conserva apenas métricas normalizadas, proveniência da estação, instante, cache e classificação temporal/origem do valor. Indisponibilidade nunca bloqueia a observação nem cria valores.
...
- A rota de Meteorologia usa exclusivamente `GET .../weather/subjects/plantation/{plantationId}/series`, paginado a 31 dias até 366 dias. O GeroFarm persiste a série normalizada sem duplicação e calcula localmente ET₀, radiação integrada, PAR/DLI, graus-dia, frio e molhamento foliar.
- Não existe consumo do endpoint Core legado de acumulações. O Core conserva somente credenciais, chamada ao fornecedor, cache, normalização e proveniência técnica; o GeroFarm conserva a associação a plantações/campanhas e toda a derivação agronómica.
...
## Meteorologia agronómica local com dados base do Core

- O consumidor GeroFarm foi revisto contra o contrato Core funcional `22968907be804edc8f591857a9aae0d1feef1881` (integrado em `c604c093331b019b7fbd4f9e500e11fecce6c3f8`), `/openapi/weather-v2.json` e os tipos provider-independent; não existe segredo, SDK, URL ou payload bruto específico do fornecedor no GeroFarm.
- A utilização publicada depende da disponibilidade desse contrato Core e da aplicação controlada da migração local `0018_local_agronomic_weather`. Esta fase não executa migrações nem validação de produção.
