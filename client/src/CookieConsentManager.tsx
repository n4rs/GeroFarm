...
export default function CookieConsentManager() {
  ...
  return <>
    ...
    {open && <div className="cookie-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section className="cookie-dialog" role="dialog" aria-modal="true" aria-labelledby="cookie-title"><button className="cookie-close" aria-label={t("cookies.manage")} onClick={() => setOpen(false)}>×</button><h2 id="cookie-title">{t("cookies.preferences.title")}</h2><p>{t("cookies.preferences.description")}</p>{gpc && <aside>{t("cookies.gpcExplanation")}</aside>}<div className="cookie-categories"><Category title={t("cookies.category.necessary.title")} description={t("cookies.category.necessary.description")} checked disabled badge={t("cookies.alwaysActive")}/><Category title={t("cookies.category.preferences.title")} description={t("cookies.currentlyInactive")} checked={choices.preferences} onChange={(preferences) => setChoices({ ...choices, preferences })} badge={t("cookies.currentlyInactive")}/><Category title={t("cookies.category.analytics.title")} description={t("cookies.category.analytics.description")} checked={choices.analytics} onChange={(analytics) => setChoices({ ...choices, analytics })} badge={t("cookies.currentlyInactive")}/><Category title={t("cookies.category.marketing.title")} description={t("cookies.category.marketing.description")} checked={choices.marketing} disabled={gpc} onChange={(marketing) => setChoices({ ...choices, marketing })} badge={t("cookies.currentlyInactive")}/></div><a href={policyHref}>{t("cookies.policyLink")}</a><div className="cookie-actions"><button className="button outline" onClick={() => apply(essentialOnly, "preferences")}>{t("cookies.rejectOptional")}</button><button className="button ghost" onClick={() => apply(all, "preferences")}>{t("cookies.acceptAll")}</button><button className="button" onClick={() => apply(choices, "preferences")}>{t("cookies.save")}</button></div></section></div>}
  </>;
}
