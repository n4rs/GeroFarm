...
test("sector creation is an explicit action in every locale",()=>{assert.equal(irrigationCopies["pt-PT"].addSector,"Criar setor");assert.equal(irrigationCopies.en.addSector,"Create sector");for(const locale of supportedLocales)assert.notEqual(irrigationCopies[locale].addSector,irrigationCopies[locale].sectors,locale)});
