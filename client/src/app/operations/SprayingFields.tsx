...
export default function SprayingFields({ t, value, set, workers, destinations, isSpraying }: Props) {
  ...
  <label><span>{t.legalApplicator}</span><select value={value.legalApplicatorWorkerId} onChange={event => set("legalApplicatorWorkerId", event.target.value)}><option value="">{t.noLegalApplicator}</option>{workers.filter(worker => worker.status === "active").map(worker => <option key={worker.id} value={worker.id}>{worker.name}</option>)}</select><small>{t.applicatorNotice}</small></label>
