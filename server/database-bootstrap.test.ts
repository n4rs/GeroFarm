...
test("fertilization plans preserve immutable versions and field snapshots",()=>{const migration=readFileSync(resolve("migrations/0013_fertilization_plans.sql"),"utf8");assert.match(migration,/fertilization_plans_org_culture_version_unique/);assert.match(migration,/fertilization_plans_one_in_force/);assert.match(migration,/fertilization_plan_fields_plan_field_unique/);assert.match(migration,/protect_fertilization_plan_history/);assert.match(migration,/protect_fertilization_plan_field_history/);assert.match(migration,/nitrate_analysis_snapshot/);assert.match(migration,/REVOKE DELETE ON TABLE "farm"\."fertilization_plans"/);assert.doesNotMatch(migration,/operation_id/)});
test("irrigation migration preserves one physical operation and auditable meter history",()=>{const migration=readFileSync(resolve("migrations/0015_irrigation.sql"),"utf8");for(const table of["irrigation_sectors","irrigation_sector_fields","water_meters","water_meter_readings","irrigation_water_analyses","irrigation_records"])assert.match(migration,new RegExp(`${table}_tenant_isolation`));assert.match(migration,/irrigation_sector_fields_active_field_unique/);assert.match(migration,/operation_id" uuid REFERENCES "farm"\."operations"/);assert.match(migration,/protect_irrigation_physical_history/);assert.match(migration,/REVOKE DELETE ON TABLE "farm"\."irrigation_records", "farm"\."water_meter_readings"/);assert.match(migration,/meter_allocations" jsonb/);assert.doesNotMatch(migration,/estimated_allocation|automatic_split/)});
