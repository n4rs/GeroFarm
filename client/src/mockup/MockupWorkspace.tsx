...
const operationDetails: Record<string, { field: string; value: string; note: string }> = {
  "Instalar cultura": { field: "Método de instalação", value: "Plantação", note: "Cria a plantação e liga-a à mesma operação física." },
  "Preparação do solo": { field: "Ações realizadas", value: "Subsolagem · Gradagem", note: "Várias ações podem pertencer à mesma passagem." },
  "Pulverização": { field: "Produto e aplicador", value: "Enxofre molhável · Maria Silva", note: "A certificação é validada na data da operação." },
  "Aplicação de produtos": { field: "Produto aplicado", value: "Bioestimulante foliar", note: "As autorizações e quantidades ficam no registo único." },
  "Fertilização": { field: "Produto e dose", value: "NPK 12-8-16 · 240 kg/ha", note: "Composição e nutrientes são preservados como histórico." },
  "Rega ou fertirrega": { field: "Origem hidráulica", value: "Setor Norte · 18,4 mm", note: "Água e nutrientes são materializados uma única vez." },
  "Trabalho cultural": { field: "Trabalhos realizados", value: "Desponta · Monda", note: "Os recursos comuns não são repetidos por destino." },
  "Monitorização": { field: "Tipo de observação", value: "Vigor e pragas", note: "Achados, amostras e meteorologia ficam rastreáveis." },
  "Colheita": { field: "Quantidade e lote", value: "12 480 kg · OLV-01", note: "Um lote físico pode ter várias origens e destinos." },
  "Outra operação": { field: "Designação", value: "Manutenção de margem", note: "Mantém o mesmo modelo de recursos, destinos e auditoria." },
};
...
export default function MockupWorkspace() {
  ...
  return <div className="farm-app">
    ...
    {selectedOperation && <div className="modal-backdrop" onMouseDown={() => setSelectedOperation(null)}><section className="operation-modal" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="specialist-operation-title">
      <div className="modal-head"><div><p>Operação realizada</p><h2 id="specialist-operation-title">{selectedOperation}</h2><span>Um único registo físico, projetado nas vistas especializadas.</span></div><button onClick={() => setSelectedOperation(null)} aria-label="Fechar">×</button></div>
      <div className="operation-grid"><label><span>Data e hora</span><input value="24/08/2026 · 10:30" readOnly /></label><label><span>Destino</span><input value="OLV-01 · Olival Norte · 100%" readOnly /></label><label><span>{operationDetails[selectedOperation].field}</span><input value={operationDetails[selectedOperation].value} readOnly /></label><label><span>Recursos comuns</span><input value="2 trabalhadores · Trator T-04" readOnly /></label></div>
      <p>{operationDetails[selectedOperation].note}</p><div className="modal-head"><button className="wide-outline" onClick={() => setSelectedOperation(null)}>Cancelar</button><button className="primary-action" onClick={() => { setSelectedOperation(null); setNotice(`${selectedOperation} validada na demonstração sem guardar dados.`); }}>Validar demonstração</button></div>
    </section></div>}
...
function GenericScreen({ screen }: { screen: Screen }) { const content: Partial<Record<Screen,{tabs:string[];cards:{label:string;value:string;note:string}[];detail:{kicker:string;title:string;description:string}}>>={planos:{tabs:["Fertilização","Rega semanal","Histórico"],cards:[{label:"Planos em vigor",value:"5",note:"3 fertilização · 2 rega"},{label:"Semana atual",value:"2 560 m³",note:"programados"},{label:"Avisos",value:"2",note:"não bloqueantes"}],detail:{kicker:"Plano de fertilização · Olival Norte",title:"Necessidades e fontes previstas",description:"A análise congelada, os objetivos e as fontes planeadas permanecem separados dos nutrientes efetivamente aplicados."}},gestao:{tabs:["Trabalhadores","Prestadores","Equipamentos","Documentos"],cards:[{label:"Trabalhadores ativos",value:"12",note:"3 aplicadores válidos"},{label:"Equipamentos",value:"18",note:"2 manutenções próximas"},{label:"Prestadores",value:"4",note:"todos ativos"}],detail:{kicker:"Validades",title:"Certificados e equipamentos a rever",description:"Maria Silva mantém certificado de aplicadora válido; o trator T-04 tem manutenção prevista para 2 de setembro."}},inventario:{tabs:["Existências","Movimentos","Compras","Por regularizar"],cards:[{label:"Produtos ativos",value:"86",note:"12 com lote obrigatório"},{label:"Valor atual",value:"24 680 €",note:"sem IVA"},{label:"Por regularizar",value:"3",note:"consumos sem stock"}],detail:{kicker:"Consumos por regularizar",title:"Três movimentos aguardam afetação",description:"As operações permanecem válidas; cada consumo será associado a uma compra ou lote sem voltar a descontar stock."}},custos:{tabs:["Resumo","Operações","Campanhas","Custos fixos"],cards:[{label:"Custo da campanha",value:"48 720 €",note:"580 €/ha"},{label:"Receita",value:"73 940 €",note:"dados registados"},{label:"Margem",value:"25 220 €",note:"34,1%"}],detail:{kicker:"Campanha 2026",title:"Custos distribuídos por área intervencionada",description:"Mão de obra, equipamentos e produtos comuns são imputados uma vez e distribuídos pelos destinos da operação."}},configuracoes:{tabs:["Organização","Catálogos","Módulos","Unidades","Idiomas"],cards:[{label:"Plano",value:"Professional",note:"até 500 ha"},{label:"Módulos ativos",value:"2",note:"Inventário e Custos"},{label:"Idioma",value:"Português",note:"Portugal"}],detail:{kicker:"Preferências centrais",title:"Idioma e acesso geridos na conta Gero",description:"O GeroFarm apresenta a organização, as permissões, o plano e os módulos recebidos do serviço central sem criar cópias locais."}}}; const data=content[screen]||content.planos!; return <><section className="section-tabs">{data.tabs.map((tab,index)=><button className={index===0?"active":""} key={tab}>{tab}</button>)}</section><section className="metric-grid three">{data.cards.map(card=><Metric key={card.label} label={card.label} value={card.value} note={card.note} trend="Ver detalhe →" />)}</section><section className="panel empty-detail"><div className="detail-graphic"><i/><i/><i/></div><div><span>{data.detail.kicker}</span><h3>{data.detail.title}</h3><p>{data.detail.description}</p></div></section></>; }
