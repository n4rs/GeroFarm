...
export const soilPreparationSchema = z.object({ actions: z.array(z.string().trim().min(2).max(120)).min(1).max(20), soilAnalysisResultId: z.string().uuid().optional(), soilAnalysisResultIdsByField:z.array(z.object({fieldId:z.string().uuid(),resultId:z.string().uuid()})).max(100).default([]), depthCm: z.number().positive().max(300).optional(), passes: z.number().int().positive().max(100).optional(), soilCondition: z.enum(soilConditions).optional(), residueDestination: z.enum(residueDestinations).optional() }).superRefine((value,context)=>{if(new Set(value.soilAnalysisResultIdsByField.map(row=>row.fieldId)).size!==value.soilAnalysisResultIdsByField.length)context.addIssue({code:"custom",path:["soilAnalysisResultIdsByField"],message:"Each field can select one soil analysis"});if(value.soilAnalysisResultId&&value.soilAnalysisResultIdsByField.length)context.addIssue({code:"custom",path:["soilAnalysisResultId"],message:"Legacy and per-field analysis choices cannot be combined"});});
...
export type OperationDto = Omit<CreateOperationInput, "workerIds" | "equipmentIds" | "contractorIds" | "workerAssignments" | "equipmentAssignments" | "contractorAssignments"> & {
    ...
    soilAnalysisSnapshots?: Array<{ fieldId:string;resultId: string; sampleId: string; sampleFieldIds:string[];sampledOn: string; resultedOn: string; validUntil?: string; laboratory: string; bulletinNumber: string; results: Array<Record<string, unknown>> }>;
    soilAnalysisWarnings?: Array<{fieldId:string;code:"missing_valid_analysis"}>;
    createdAt: string;
